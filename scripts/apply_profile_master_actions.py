...
for setter, deleter, table in [
    ...
    if f"export async function {deleter}" not in s:
        s = insert_after_line(
            s,
            f"export async function {setter}",
            f"export async function {deleter}(id:number){{await db.query(`DELETE FROM {table} WHERE id=?`,[id]);}}",
        )
p.write_text(s)
...
for old, new in [
    ...
    if new not in s:
        if old not in s:
            raise SystemExit(f"Missing service import anchor: {old}")
...
for fn, dbfn, label, anchor in [
    ("removeOrgUnit", "deleteOrgUnit", "Organization ID", "addOrgUnit"),
    ("removeBank", "deleteBank", "Bank ID", "addBank"),
    ("removeDesignation", "deleteDesignation", "Designation ID", "addDesignation"),
    ("removeStream", "deleteStream", "Stream ID", "addStream"),
    ("removeMscBatch", "deleteMscBatch", "MSc Batch ID", "addMscBatch"),
    ("removeCourse", "deleteCourse", "Course ID", "addCourse"),
    ("removeTraineeBatch", "deleteTraineeBatch", "Trainee Batch ID", "addTraineeBatch"),
]:
    if f"export const {fn}=" not in s:
        s = s.replace(
            f"export async function {anchor}",
            f'export const {fn}=(value:unknown)=>removeRecord({dbfn},value,"{label}");\nexport async function {anchor}',
            1,
        )
p.write_text(s)
...
for fn, svc, label, anchor in [
    ("deleteOrgUnit", "removeOrgUnit", "Organization", "createOrgUnit"),
    ("deleteBank", "removeBank", "Bank", "createBank"),
    ("deleteDesignation", "removeDesignation", "Designation", "createDesignation"),
    ("deleteStream", "removeStream", "Stream", "createStream"),
    ("deleteMscBatch", "removeMscBatch", "MSc batch", "createMscBatch"),
    ("deleteCourse", "removeCourse", "Course", "createCourse"),
...
s = p.read_text()
for path, fn in [
    ("employment-types", "deleteEmploymentType"),
    ("org-units", "deleteOrgUnit"),
    ("banks", "deleteBank"),
    ("designations", "deleteDesignation"),
    ("streams", "deleteStream"),
    ("msc-batches", "deleteMscBatch"),
    ("courses", "deleteCourse"),
    ("trainee-batches", "deleteTraineeBatch"),
]:
    if f"controller.{fn}" not in s:
        anchor = '  app.get("/api/admin/profile-masters/employment-types"' if path == "employment-types" else f'  app.patch("/api/admin/profile-masters/{path}/:id/status"'
        pos = s.find(anchor)
        if pos < 0:
            raise SystemExit(f"Missing route anchor: {path}")
        line_start = s.rfind("  ", 0, pos)
        line = f'  app.delete("/api/admin/profile-masters/{path}/:id", authenticateToken, administrator, controller.{fn});\n'
        s = s[:line_start] + line + s[line_start:]
p.write_text(s)
...
for suffix, path in [
    ...
    ("Stream", "streams"),
    ("MscBatch", "msc-batches"),
    ("Course", "courses"),
    ("TraineeBatch", "trainee-batches"),
]:
    ...
    if f"export const {fn}=" not in s:
        ...
        if anchor not in s:
            raise SystemExit(f"Missing API anchor: {suffix}")
...
if old not in s:
    raise SystemExit("UI master table anchor not found")
s = s.replace(old, new, 1)
...
if legacy not in s:
    raise SystemExit("Legacy ActionCell anchor not found")
s = s.replace(legacy, "", 1)
p.write_text(s)

# Restore the normal CI workflow and remove all temporary bridge files.
