        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-2xs p-3.5 sm:p-5 space-y-3.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 text-xs">
            ...
            {employmentCode === "contractual" && <>
              <div><label className="block font-semibold text-slate-700 mb-1">Designation<span className="profile-required-star">*</span></label><input required value={profile.designation || ""} onChange={(e) => handleChange("designation", e.target.value)} className={getFieldClass()} /></div>
              <div><label className="block font-semibold text-slate-700 mb-1">Project / Department / Cell<span className="profile-required-star">*</span></label><select required value={profile.projectId ?? ""} onChange={(e) => handleChange("projectId", e.target.value ? Number(e.target.value) : null)} className={getFieldClass()}><option value="">Select Project / Department / Cell</option>{[...projectUnits,...departmentUnits,...cellUnits].map((unit) => <option key={unit.id} value={unit.id}>{unit.unitName}</option>)}</select></div>
            ...
            {employmentCode === "researcher_project_staff" && <>
              <div><label className="block font-semibold text-slate-700 mb-1">Designation<span className="profile-required-star">*</span></label><input required value={profile.designation || ""} onChange={(e) => handleChange("designation", e.target.value)} className={getFieldClass()} /></div>
              <div><label className="block font-semibold text-slate-700 mb-1">Project<span className="profile-required-star">*</span></label><select required value={profile.projectId ?? ""} onChange={(e) => handleChange("projectId", e.target.value ? Number(e.target.value) : null)} className={getFieldClass()}><option value="">Select Project</option>{projectUnits.map((unit) => <option key={unit.id} value={unit.id}>{unit.unitName}</option>)}</select></div>
