...
export const ProfileForm: React.FC<ProfileFormProps> = ({
  ...
  const [profile, setProfile] = useState<ApplicantProfile>({
    profilePhotoUrl:
      ...
      SAMPLE_USER_PROFILES["USR-002"].profilePhotoUrl,
    ...
    officeOrderFileName:
      initialProfile.officeOrderFileName ||
      "WII_Office_Order_Engagement_2026.pdf",
    ...initialProfile,
  });
  ...
  const filteredUserEntries = Object.entries(SAMPLE_USER_PROFILES).filter(
    ([id, p]) => {
      ...
      return (
        ...
        (p.departmentCellProject &&
          p.departmentCellProject.toLowerCase().includes(q)) ||
        (p.employmentType && p.employmentType.toLowerCase().includes(q)) ||
        (p.bankName && p.bankName.toLowerCase().includes(q)) ||
        (p.supervisingOfficerName &&
          p.supervisingOfficerName.toLowerCase().includes(q))
      );
    },
  );
  ...
  const handleSelectUser = (userId: string) => {
    ...
    const targetProfile =
      SAMPLE_USER_PROFILES[userId] || SAMPLE_USER_PROFILES["USR-002"];
  ...
  const validate = (data: ApplicantProfile): Record<string, string> => {
    ...
    if (!data.ifscCode?.trim()) {
      ...
      errs.ifscCode =
        "Enter a valid 11-character IFSC Code (e.g. HDFC0000123).";
    }
    ...
    if (!data.departmentCellProject?.trim()) {
      ...
      errs.departmentCellProject =
        "Project / Department name must be at least 2 characters.";
    }

    // Reporting Officer / Supervising Officer (PI)
    if (!data.supervisingOfficerName?.trim()) {
      errs.supervisingOfficerName =
        "Reporting / Supervising Officer (PI) is required.";
    } else if (data.supervisingOfficerName.trim().length < 3) {
      errs.supervisingOfficerName =
        "Officer name must be at least 3 characters.";
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    recordSecurityAuditLog({
      ...
      actorRole: (currentRole === "super_admin"
        ? "admin"
        : currentRole || "applicant") as UserRole,
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-8">
        ...
        <div className="z-10 relative space-y-1.5 max-w-2xl min-w-0 flex-1">
          ...
          <h1 className="text-lg sm:text-2xl font-extrabold tracking-tight text-white leading-snug sm:leading-tight break-words">
            {currentRole === "applicant"
              ...
              : "Directory Lookup & Profile Inspection"}
          </h1>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden max-h-72 overflow-y-auto">
                  ...
                  {filteredUserEntries.length === 0 ? (
                    <div className="p-3 text-xs text-slate-400 text-center">
                      No matching personnel profile found for &quot;
                      {profileSearchQuery}&quot;
                    </div>
                  ) : (
                    filteredUserEntries.map(([id, p]) => (
                      <button
                        ...
                        className={`w-full text-left p-2.5 hover:bg-slate-800 flex items-center gap-2.5 border-b border-slate-800/60 transition-colors cursor-pointer ${
                          selectedUserId === id
                            ? "bg-slate-800/90 border-l-4 border-l-emerald-400"
                            : ""
                        }`}
                      >
                        <img
      ...
      {Object.keys(errors).length > 0 && (
        <div className="bg-rose-50 border border-rose-200 text-rose-800 p-3.5 rounded-xl flex items-center gap-2.5 text-xs font-medium">
          ...
          <div>
            <strong className="font-bold">Form validation error:</strong> Please
            correct the highlighted fields below before submitting.
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 text-xs">
            ...
            <div>
              ...
              <input
                ...
                disabled={!isEditable}
                onChange={(e) =>
                  handleChange("departmentCellProject", e.target.value)
                }
                className={`w-full text-xs px-3 py-2 border rounded-md font-medium text-slate-800 disabled:bg-slate-100 ${
            ...
            <div>
              ...
              <input
                ...
                disabled={!isEditable}
                onChange={(e) =>
                  handleChange("supervisingOfficerName", e.target.value)
                }
            ...
            <div>
              <label className="block font-semibold text-slate-700 mb-1">
                Project / Fellowship Valid Up To{" "}
                <span className="text-rose-500">*</span>
              </label>
              <input
                type="text"
