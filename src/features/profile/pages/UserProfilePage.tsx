...
const NO_BANK_EMPLOYMENT_TYPES = new Set(["msc_student", "diploma_trainee", "intern"]);
const PERMANENT_CODE = "permanent";

function normalizeEmploymentCode(value: string | null | undefined): string {
  return String(value || "").trim().toLowerCase().replace(/&/g, "and").replace(/[\\/]/g, "_").replace(/\s+/g, "_").replace(/-+/g, "_");
}
...
function emptyProfile(): FormState {
  return {
    userId: "", profilePhotoPath: null, salutation: "", applicantName: "", employmentType: "", gender: "",
    dateOfBirth: "", bloodGroup: "", mobileNo: "", personalEmail: "", wiiOfficialEmail: "", address: "",
    city: "", state: "", pincode: "", designation: "", stream: "", courseName: "", departmentCellProject: "",
    departmentId: null, projectId: null, supervisingOfficerId: null, supervisingOfficerName: "", reportingOfficerId: null,
    reportingManagerId: null, piUserId: null, batchId: null, dateOfJoining: "", validUpTo: "", panNo: "",
    bankName: "", accountNo: "", ifscCode: "", officeOrderFileName: null, biometricId: null,
  };
}

function toFormState(profile: ApplicantProfile): FormState {
  return { ...profile, dateOfBirth: toDateInput(profile.dateOfBirth), dateOfJoining: toDateInput(profile.dateOfJoining), validUpTo: toDateInput(profile.validUpTo) };
}

function isAdminRole(role: string): boolean {
  return ["admin", "administrator", "super_admin", "system_administrator"].includes(role.toLowerCase());
}
...
function getUnits(units: ProfileOrgUnit[], type: "department" | "cell" | "project"): ProfileOrgUnit[] {
...
export const ProfileForm: React.FC<ProfileFormProps> = ({ initialProfile, currentRole = "applicant", onSaveProfile }) => {
  const [profile, setProfile] = useState<FormState>(initialProfile ? toFormState(initialProfile as unknown as ApplicantProfile) : emptyProfile());
  ...
  const activeEmploymentTypes = useMemo(() => employmentTypes.filter((x) => String((x as any).status ?? "active").toLowerCase() === "active"), [employmentTypes]);
  ...
  useEffect(() => {
    ...
    if (pin !== profile.pincode) {
      setProfile((previous) => ({ ...previous, pincode: pin, ...(pin.length < 6 ? { city: "", state: "" } : {}) }));
      return;
    }
    if (pin.length !== 6) { setPincodeLoading(false); setPincodeError(""); return; }
    let cancelled = false;
    setPincodeLoading(true); setPincodeError("");
    void getPincodeDetails(pin).then((details) => {
      if (!cancelled) setProfile((previous) => ({ ...previous, city: details.district, state: details.state }));
    }).catch((error) => {
      if (!cancelled) {
        const message = error instanceof Error ? error.message : "Unable to verify PIN code.";
        setPincodeError(message); setSaveError(message); setProfile((previous) => ({ ...previous, city: "", state: "" }));
      }
  ...
  const handleProfilePhotoChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    event.target.value = "";
    if (!file) return;
    setPhotoError(""); setSaveError(""); setValidationAttempted(false);
    const allowedTypes = new Set(["image/jpeg", "image/png", "image/webp"]);
    if (!allowedTypes.has(file.type)) { setPhotoError("Please select a JPG, PNG, or WebP image."); return; }
    ...
    reader.onerror = () => setPhotoError("Unable to preview the selected profile photo.");
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setProfile(toFormState(savedProfile)); onSaveProfile?.(savedProfile as unknown as LegacyApplicantProfile);
      setIsSaved(true); setValidationAttempted(false); window.setTimeout(() => setIsSaved(false), 3000);
    } catch (error) {
      setSaveError(error instanceof Error ? error.message : "Unable to save profile.");
    } finally { setSaving(false); }
  };

  const getFieldClass = (extra = "") => `w-full text-xs px-3 py-2 border border-slate-300 rounded-md text-slate-800 disabled:cursor-not-allowed ${extra}`;
  const getImmutableFieldClass = (extra = "") => `${getFieldClass(extra)} bg-slate-200 text-slate-600 cursor-not-allowed`;
  const getAutoFieldClass = (extra = "") => `${getFieldClass(extra)} bg-white text-slate-800`;
  const renderOfficerOptions = () => officers.map((officer) => <option key={officer.id} value={officer.id}>{officer.fullName}</option>);

  if (loading) return <div className="max-w-7xl mx-auto py-12 flex justify-center"><div className="text-sm text-slate-500">Loading profile...</div></div>;
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-8">
      <style>{`
        ...
        .profile-form.validation-attempted select:invalid:not(:disabled) { border-color:#fca5a5; box-shadow:0 0 0 1px rgba(248,113,113,.12); }
        .profile-form input:read-only { cursor:not-allowed; }
      ...
      <form onSubmit={handleSubmit} className={`profile-form space-y-5 ${validationAttempted ? "validation-attempted" : ""}`} noValidate>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-2xs p-3.5 sm:p-5 space-y-4">
          <div className="border-b border-slate-100 pb-2.5"><h2 className="text-xs font-bold uppercase text-slate-800 tracking-wider flex items-center gap-2"><User className="w-4 h-4 text-emerald-600" />1. Personal Details</h2></div>
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-5">
            ...
            <div className="lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 text-xs">
              ...
              <div><label className="block font-semibold text-slate-700 mb-1">Employment Type<span className="profile-required-star">*</span></label><select required value={profile.employmentType || ""} onChange={(e) => handleEmploymentChange(e.target.value)} className={getFieldClass()}><option value="">Select Employment Type</option>{activeEmploymentTypes.map((item) => <option key={item.id} value={item.displayName}>{item.displayName}</option>)}</select></div>
              ...
              <div><label className="block font-semibold text-slate-700 mb-1">Mobile<span className="profile-required-star">*</span></label><input required type="tel" inputMode="numeric" maxLength={15} pattern="[0-9]{10,15}" value={profile.mobileNo || ""} disabled={!adminCanEditOfficialFields} onChange={(e) => handleChange("mobileNo", e.target.value.replace(/\D/g, "").slice(0,15))} className={`${getImmutableFieldClass("font-mono")} immutable-field`} /></div>
              <div className="sm:col-span-2"><label className="block font-semibold text-slate-700 mb-1">Personal Email<span className="profile-required-star">*</span></label><input required type="email" value={profile.personalEmail || ""} disabled={!adminCanEditOfficialFields} onChange={(e) => handleChange("personalEmail", e.target.value)} className={`${getImmutableFieldClass("font-mono")} immutable-field`} /></div>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-xl border border-slate-200 shadow-2xs p-3.5 sm:p-5 space-y-3.5">
          <div className="border-b border-slate-100 pb-2.5"><h2 className="text-xs font-bold uppercase text-slate-800 tracking-wider flex items-center gap-2"><MapPin className="w-4 h-4 text-emerald-600" />2. Address Details</h2></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3.5 text-xs">
            <div className="sm:col-span-2 lg:col-span-2"><label className="block font-semibold text-slate-700 mb-1">Permanent Address<span className="profile-required-star">*</span></label><input required value={profile.address || ""} onChange={(e) => handleChange("address", e.target.value)} className={getFieldClass()} /></div>
            <div><label className="block font-semibold text-slate-700 mb-1">Pin Code<span className="profile-required-star">*</span></label><input required inputMode="numeric" maxLength={6} pattern="[0-9]{6}" value={profile.pincode || ""} onChange={(e) => handleChange("pincode", e.target.value.replace(/\D/g, "").slice(0,6))} className={getFieldClass("font-mono")} />{pincodeLoading && <p className="text-[10px] text-slate-500 mt-1">Finding location...</p>}</div>
            <div><label className="block font-semibold text-slate-700 mb-1">City / District<span className="profile-required-star">*</span></label><input required readOnly value={profile.city || ""} className={`${getImmutableFieldClass()} immutable-field`} /></div>
            <div><label className="block font-semibold text-slate-700 mb-1">State<span className="profile-required-star">*</span></label><input required readOnly value={profile.state || ""} className={`${getImmutableFieldClass()} immutable-field`} /></div>
          </div>
        </div>

        {isBankVisible && <div className="bg-white rounded-xl border border-slate-200 shadow-2xs p-3.5 sm:p-5 space-y-3.5">
          <div className="border-b border-slate-100 pb-2.5"><h2 className="text-xs font-bold uppercase text-slate-800 tracking-wider flex items-center gap-2"><CreditCard className="w-4 h-4 text-emerald-600" />3. Bank Details</h2></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5 text-xs">
            <div><label className="block font-semibold text-slate-700 mb-1">PAN<span className="profile-required-star">*</span></label><input required maxLength={10} pattern="[A-Za-z]{5}[0-9]{4}[A-Za-z]" value={profile.panNo || ""} onChange={(e) => handleChange("panNo", e.target.value.toUpperCase())} className={getFieldClass("font-mono uppercase")} /></div>
            <div><label className="block font-semibold text-slate-700 mb-1">Bank<span className="profile-required-star">*</span></label><select required value={profile.bankName || ""} onChange={(e) => handleChange("bankName", e.target.value)} className={getFieldClass()}><option value="">Select Bank</option>{activeBanks.map((bank) => <option key={bank.id} value={bank.bankName}>{bank.bankName}</option>)}</select></div>
            <div><label className="block font-semibold text-slate-700 mb-1">Account Number<span className="profile-required-star">*</span></label><input required inputMode="numeric" maxLength={30} pattern="[0-9]{6,30}" value={profile.accountNo || ""} onChange={(e) => handleChange("accountNo", e.target.value.replace(/\D/g, ""))} className={getFieldClass("font-mono")} /></div>
            <div><label className="block font-semibold text-slate-700 mb-1">IFSC Code<span className="profile-required-star">*</span></label><input required maxLength={11} pattern="[A-Za-z]{4}0[A-Za-z0-9]{6}" value={profile.ifscCode || ""} onChange={(e) => handleChange("ifscCode", e.target.value.toUpperCase())} className={getFieldClass("font-mono uppercase")} /></div>
          </div>
        </div>}
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-2xs p-3.5 sm:p-5 space-y-3.5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5 text-xs">
            {(employmentCode === "permanent" || employmentCode === "deputation") && <>
              ...
              <div><label className="block font-semibold text-slate-700 mb-1">Department / Cell<span className="profile-required-star">*</span></label><select required value={profile.departmentId ?? ""} onChange={(e) => handleChange("departmentId", e.target.value ? Number(e.target.value) : null)} className={getFieldClass()}><option value="">Select Department / Cell</option>{[...getUnits(activeOrgUnits, "department"),...getUnits(activeOrgUnits, "cell")].map((unit) => <option key={unit.id} value={unit.id}>{unit.unitName}</option>)}</select></div>
            ...
            {employmentCode === "contractual" && <>
              ...
              <div><label className="block font-semibold text-slate-700 mb-1">Project / Department / Cell<span className="profile-required-star">*</span></label><select required value={profile.projectId ?? ""} onChange={(e) => handleChange("projectId", e.target.value ? Number(e.target.value) : null)} className={getFieldClass()}><option value="">Select Project / Department / Cell</option>{[...getUnits(activeOrgUnits, "project"),...getUnits(activeOrgUnits, "department"),...getUnits(activeOrgUnits, "cell")].map((unit) => <option key={unit.id} value={unit.id}>{unit.unitName}</option>)}</select></div>
              <div><label className="block font-semibold text-slate-700 mb-1">Reporting Manager / PI<span className="profile-required-star">*</span></label><select required value={profile.reportingManagerId ?? ""} onChange={(e) => handleChange("reportingManagerId", e.target.value ? Number(e.target.value) : null)} className={getFieldClass()}><option value="">Select Reporting Manager / PI</option>{renderOfficerOptions()}</select></div>
            </>}
            {employmentCode === "researcher_project_staff" && <>
              <div><label className="block font-semibold text-slate-700 mb-1">Designation<span className="profile-required-star">*</span></label><input required value={profile.designation || ""} onChange={(e) => handleChange("designation", e.target.value)} className={getFieldClass()} /></div>
              <div><label className="block font-semibold text-slate-700 mb-1">Project<span className="profile-required-star">*</span></label><select required value={profile.projectId ?? ""} onChange={(e) => handleChange("projectId", e.target.value ? Number(e.target.value) : null)} className={getFieldClass()}><option value="">Select Project</option>{getUnits(activeOrgUnits, "project").map((unit) => <option key={unit.id} value={unit.id}>{unit.unitName}</option>)}</select></div>
            ...
            {employmentCode === "msc_student" && <>
              <div><label className="block font-semibold text-slate-700 mb-1">Stream<span className="profile-required-star">*</span></label><select required value={profile.streamId ?? ""} onChange={e=>{const id=e.target.value?Number(e.target.value):null;const x=streams.find(v=>Number(v.id)===id);setProfile(p=>({...p,streamId:id,stream:x?.streamName||"",mscBatchId:null,batchId:null}));}} className={getFieldClass()}><option value="">Select Stream</option>{activeStreams.map(x=><option key={x.id} value={x.id}>{x.streamName}</option>)}</select></div>
            ...
            {employmentCode === "diploma_trainee" && <>
              <div><label className="block font-semibold text-slate-700 mb-1">Course Name<span className="profile-required-star">*</span></label><select required value={profile.courseId ?? ""} onChange={e=>{const id=e.target.value?Number(e.target.value):null;const x=courses.find(v=>Number(v.id)===id);setProfile(p=>({...p,courseId:id,courseName:x?.courseName||"",traineeBatchId:null,batchId:null}));}} className={getFieldClass()}><option value="">Select Course</option>{activeCourses.map(x=><option key={x.id} value={x.id}>{x.courseName}</option>)}</select></div>
            ...
            <div><label className="block font-semibold text-slate-700 mb-1">Valid Up To<span className="profile-required-star">*</span></label><input required type="date" value={profile.validUpTo || ""} readOnly={isPermanent(profile.employmentType || "")} onChange={(e) => handleChange("validUpTo", e.target.value)} className={getAutoFieldClass()} />{isPermanent(profile.employmentType || "") && <p className="text-[10px] text-slate-500 mt-1">Automatically calculated by the server from Date of Birth as the last day of the month in which the user turns 60.</p>}</div>
          </div>
        </div>

        <div className="flex items-center justify-end pt-2 pb-6"><button type="submit" disabled={saving || isPhotoUploading} className="flex items-center gap-2 px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 disabled:cursor-not-allowed text-white font-bold rounded-xl shadow-xs transition-all text-xs"><Save className="w-4 h-4" />{saving ? "Saving..." : "Save Profile Changes"}</button></div>
      </form>
    </div>
  );
};
