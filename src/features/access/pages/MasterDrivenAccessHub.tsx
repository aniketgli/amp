...
export const MasterDrivenAccessHub: React.FC<Props> = ({ applicantProfile, currentRole = "applicant", requisitions, onSelectRequisition: _onSelectRequisition, onSubmitRequisition, onNavigateTab }) => {
  ...
  return <div className="space-y-6 max-w-[1220px] mx-auto pb-8">
    ...
    <section className="space-y-4"><div className="flex items-end justify-between"><div><h2 className="text-lg font-extrabold flex items-center gap-2"><Building2 className="w-5 h-5 text-purple-600" /> Facilities & Research Labs</h2><p className="text-xs text-slate-500">Facility details and popup fields are loaded from Facilities Master.</p></div><span className="px-3 py-1 rounded-full bg-purple-50 text-purple-700 text-[11px] font-bold">{facilities.length} Active</span></div>
      {loading ? <div className="p-10 text-center bg-white border rounded-xl text-xs text-slate-500">Loading Facilities Master...</div> : facilities.length === 0 ? <div className="p-10 text-center bg-white border border-dashed rounded-xl text-xs text-slate-500">No active facilities are configured.</div> : <div className="space-y-4">{facilities.map((facility) => <article key={facility.id} className="bg-white rounded-xl border border-slate-200 overflow-hidden"><div className="px-4 py-4 flex items-center justify-between gap-3 border-b bg-slate-50"><div className="flex items-center gap-3 min-w-0"><div className="w-9 h-9 rounded-lg bg-purple-600 text-white flex items-center justify-center"><FlaskConical className="w-5 h-5" /></div><div className="min-w-0"><h3 className="text-sm font-extrabold truncate">{facility.name}</h3><p className="text-[11px] text-slate-500 truncate">{facility.id} · Facility / Research Lab</p></div></div><span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-purple-50 text-purple-700">Active</span></div><div className="p-5"><div className="grid grid-cols-1 lg:grid-cols-[1fr_auto] gap-5 items-center"><div className="text-xs text-slate-600 space-y-1"><div><b>Nodal:</b> {facility.nodal || "—"}</div><div><b>Associate Nodal:</b> {facility.assocNodal || "—"}</div><div><b>Supervisor:</b> {facility.supervisor || "—"}</div>{facility.description && <div className="text-slate-500 pt-1">{facility.description}</div>}</div><button type="button" onClick={() => setModal({ facility, scope: "lab", mode: "new" })} className="w-full lg:w-auto min-w-[220px] px-5 py-3 rounded-lg bg-purple-600 hover:bg-purple-500 text-white text-xs font-bold"><PlusCircle className="inline w-3.5 h-3.5 mr-1" /> Apply for Facility Access</button></div><div className="mt-5 rounded-xl border border-dashed border-purple-200 bg-purple-50/40 p-7 text-center"><div className="text-sm font-extrabold">No Active {facility.name} Access</div><p className="text-xs text-slate-500 mt-1">{facility.formConfig?.fields?.length ? `${facility.formConfig.fields.length} form field(s) configured in Facilities Master.` : "This facility uses the master configuration available for this facility."}</p></div></div></article>)}</div>}
    </section>
