...
export const LAB_FACILITIES_LIST = [
  {
    id: "gis",
    name: "GIS & Remote Sensing Laboratory",
    dept: "RS & GIS Cell",
  ...
  {
    id: "conservation_genetics",
    name: "Conservation Genetics Facility",
    dept: "Genetics Cell",
...
export const MyAccessHub: React.FC<MyAccessHubProps> = ({
  ...
  const activeRoleInfo =
    OFFICIAL_ROLES.find((r) => r.id === currentRole) || OFFICIAL_ROLES[0];
  ...
  const approvedReqs = userRequisitions.filter(
    (r) => r.status === "approved_provisioned",
  );
  ...
  const approvedEmailReq = approvedReqs.find(
    (r) => r.itHrmsDetails?.requestEmail,
  );
  const pendingEmailReq = pendingReqs.find(
    (r) => r.itHrmsDetails?.requestEmail,
  );

  const approvedNetReq = approvedReqs.find(
    (r) => r.itHrmsDetails?.requestInternet,
  );
  const pendingNetReq = pendingReqs.find(
    (r) => r.itHrmsDetails?.requestInternet,
  );

  const approvedHrmsReq = approvedReqs.find(
    (r) => r.itHrmsDetails?.requestHrmsPms || r.itHrmsDetails?.requestBiometric,
  );
  const pendingHrmsReq = pendingReqs.find(
    (r) => r.itHrmsDetails?.requestHrmsPms || r.itHrmsDetails?.requestBiometric,
  );
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-8">
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-4 sm:p-6 border border-slate-800 shadow-md relative overflow-hidden flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-5 min-h-[140px]">
        ...
        <div className="space-y-1.5 max-w-2xl min-w-0 flex-1 z-10 relative">
          ...
          <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight text-white leading-snug sm:leading-tight break-words">
            {currentRole === "applicant" &&
              "Personnel Requisition & Access Hub"}
            {currentRole === "supervisor" &&
              "PI & Supervising Officer Access Management Hub"}
            {currentRole === "lab_nodal" &&
              "WII Research Laboratories Nodal Operations Desk"}
            {currentRole === "it_officer" &&
              "IT & Network Infrastructure Provisioning Desk"}
            {currentRole === "hrms_officer" &&
              "HRMS ERP & Biometric Attendance Services Hub"}
            {currentRole === "section_head" &&
              "Section Head Executive Access Oversight"}
            {currentRole === "admin" &&
              "Central Security & System Access Command Center"}
          </h1>
        ...
        <div className="flex flex-row items-center sm:flex-col sm:items-end gap-2 sm:gap-2.5 z-10 relative shrink-0 w-full sm:w-auto min-w-0">
          <button
            type="button"
            onClick={() =>
              onNavigateTab ? onNavigateTab("my_requests") : null
            }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-blue-900">
                Official Role Mode Active: {activeRoleInfo.title}
              </span>
              <p className="text-blue-700 text-[11px]">
                You are viewing the Access Services Hub under{" "}
                <span className="font-semibold">{activeRoleInfo.title}</span>{" "}
                permissions. Actions and pass views are tailored for your
                administrative role.
              </p>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-500 dark:text-slate-400">
              Issued & provisioned credentials
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-[10px] text-slate-500 dark:text-slate-400">
              In verification approval chain
            </div>
          </div>
        </div>
          ...
          <div className="flex items-start sm:items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-sm font-bold text-slate-900 leading-tight">
                1. Official WII Email ID (@wii.gov.in)
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Institute Webmail Account, Domain Access & Group Mappings
              </p>
            </div>
          </div>
            ...
            <div className="p-3.5 sm:p-4 rounded-xl border border-emerald-200 bg-emerald-50/50 space-y-3 text-xs min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                <span className="font-bold text-emerald-900 flex items-center gap-1.5">
                  ...
                  Active Authorized Email Account
                </span>
                <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white p-3 rounded-lg border border-slate-200">
                <div>
                  ...
                  <span className="font-mono text-xs font-bold text-blue-700 break-all">
                    {approvedEmailReq.itHrmsDetails?.assignedWiiEmail ||
                      "Issued by IT Cell"}
                  </span>
                </div>
                <div>
                  ...
                  <span className="text-slate-700 font-semibold">
                    {approvedEmailReq.itHrmsDetails?.assignedEmailGroups?.join(
                      ", ",
                    ) || "All Staff"}
                  </span>
                </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-[11px] pt-1 min-w-0">
                <span className="text-slate-500 font-medium">
                  Requisition Ref:{" "}
                  <code className="font-mono bg-slate-200 px-1.5 py-0.5 rounded font-bold text-slate-800">
                    {approvedEmailReq.id}-EML
                  </code>
                </span>
                <a
            ...
            <div className="p-3.5 sm:p-4 rounded-xl border border-amber-200 bg-amber-50/50 space-y-2 text-xs min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                <span className="font-bold text-amber-900 flex items-center gap-1.5">
                  <Clock className="w-4 h-4 text-amber-600 shrink-0" /> Email
                  Requisition Pending Verification
                </span>
              ...
              <p className="text-slate-600 text-[11px] leading-relaxed">
                ...
                has been submitted and is currently being verified by your
                Supervising Officer / IT Cell.
              </p>
              <button
                onClick={() =>
                  onSelectRequisition({
            ...
            <div className="p-4 sm:p-6 text-center space-y-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
              ...
              <div>
                <div className="text-xs font-bold text-slate-800">
                  No Active WII Webmail Account
                </div>
                <p className="text-[11px] text-slate-500 max-w-sm mx-auto mt-0.5 leading-relaxed">
                  Apply for an official @wii.gov.in email address to access
                  institutional communications, research groups, and domain
                  resources.
                </p>
              </div>
              <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xs overflow-hidden min-w-0">
        <div className="p-3.5 sm:p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 min-w-0">
          <div className="flex items-start sm:items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-sm font-bold text-slate-900 leading-tight">
                2. Campus Internet & Wi-Fi MAC Address Registration
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Device Hardware Address MAC Binding for High-Speed LAN & Campus
                Wi-Fi
              </p>
            </div>
          </div>
          ...
          <div className="self-start sm:self-auto shrink-0">
            {approvedNetReq ? (
              ...
              <span className="px-2.5 py-1 bg-amber-100 text-amber-800 font-extrabold rounded-full text-xs border border-amber-300 flex items-center gap-1">
                <Clock className="w-3.5 h-3.5 text-amber-600 shrink-0" /> MAC
          ...
          {approvedNetReq ? (
            <div className="p-3.5 sm:p-4 rounded-xl border border-emerald-200 bg-emerald-50/50 space-y-3 text-xs min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                <span className="font-bold text-emerald-900 flex items-center gap-1.5">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />{" "}
                  Registered Device Hardware MAC
                </span>
                <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white p-3 rounded-lg border border-slate-200">
                <div>
                  ...
                  <span className="font-bold text-slate-800">
                    {approvedNetReq.itHrmsDetails?.deviceType || "Workstation"}
                  </span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px] uppercase font-bold">
                  ...
                  <code className="font-mono text-xs font-bold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-200 break-all inline-block">
                    {approvedNetReq.itHrmsDetails?.verifiedMacAddress ||
                      approvedNetReq.itHrmsDetails?.macAddress ||
                      "Registered"}
                  </code>
                </div>
                <div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-[11px] pt-1 min-w-0">
                <span className="text-slate-500 font-medium">
                  Requisition Ref:{" "}
                  <code className="font-mono bg-slate-200 px-1.5 py-0.5 rounded font-bold text-slate-800">
                    {approvedNetReq.id}-NET
                  </code>
                </span>
                <span className="text-emerald-700 font-bold flex items-center gap-1">
                  <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />{" "}
                  IT MAC Registration Verified
                </span>
              </div>
            </div>
          ) : pendingNetReq ? (
            <div className="p-3.5 sm:p-4 rounded-xl border border-amber-200 bg-amber-50/50 space-y-2 text-xs min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                <span className="font-bold text-amber-900 flex items-center gap-1.5">
                  <Clock className="w-4 h-4 text-amber-600 shrink-0" /> Campus
                  Internet MAC Registration Pending
                </span>
                <span className="self-start sm:self-auto px-2.5 py-0.5 bg-amber-100 text-amber-800 font-bold rounded-full text-[10px] border border-amber-300 shrink-0">
              ...
              <p className="text-slate-600 text-[11px] leading-relaxed">
                ...
                is pending verification by IT Cell.
              </p>
              <button
                onClick={() =>
                  onSelectRequisition({
            ...
            <div className="p-4 sm:p-6 text-center space-y-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
              ...
              <div>
                <div className="text-xs font-bold text-slate-800">
                  No Campus Internet Device Bound
                </div>
                <p className="text-[11px] text-slate-500 max-w-sm mx-auto mt-0.5 leading-relaxed">
                  Register your laptop or workstation MAC hardware address to
                  enable Wi-Fi and high-speed LAN access across WII campus.
                </p>
              </div>
              <button
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-2xs overflow-hidden min-w-0">
        <div className="p-3.5 sm:p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 min-w-0">
          <div className="flex items-start sm:items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
            ...
            <div className="min-w-0 flex-1">
              <h3 className="text-sm font-bold text-slate-900 leading-tight">
                3. HRMS / PMS Portal & Biometric Attendance
              </h3>
              <p className="text-xs text-slate-500 mt-0.5">
                Personnel Management System Portal Accounts & Biometric ID
                Mapping
              </p>
            </div>
          </div>
          ...
          {approvedHrmsReq ? (
            <div className="p-3.5 sm:p-4 rounded-xl border border-emerald-200 bg-emerald-50/50 space-y-3 text-xs min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                <span className="font-bold text-emerald-900 flex items-center gap-1.5">
                  <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />{" "}
                  Active HRMS Portal & Biometric Access
                </span>
                <button
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-white p-3 rounded-lg border border-slate-200">
                ...
                <div>
                  ...
                  <span className="font-mono text-xs font-bold text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-200 inline-block">
                    {approvedHrmsReq.itHrmsDetails?.assignedBiometricId ||
                      applicantProfile.biometricId ||
                      "WII-BIO-1048"}
                  </span>
                </div>
                <div>
            ...
            <div className="p-3.5 sm:p-4 rounded-xl border border-amber-200 bg-amber-50/50 space-y-2 text-xs min-w-0">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                <span className="font-bold text-amber-900 flex items-center gap-1.5">
                  <Clock className="w-4 h-4 text-amber-600 shrink-0" /> HRMS /
                  Biometric Requisition Pending
                </span>
              ...
              <p className="text-slate-600 text-[11px] leading-relaxed">
                ...
                is currently being reviewed for HRMS portal mapping and
                biometric punch template registration.
              </p>
              <button
                onClick={() =>
                  onSelectRequisition({
            ...
            <div className="p-4 sm:p-6 text-center space-y-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
              ...
              <div>
                <div className="text-xs font-bold text-slate-800">
                  No Active HRMS / Biometric Access
                </div>
                <p className="text-[11px] text-slate-500 max-w-sm mx-auto mt-0.5 leading-relaxed">
                  Apply for HRMS / PMS ERP portal access and Biometric
                  Attendance registration for daily Institute duty logs.
                </p>
              </div>
              <button
      ...
      {(() => {
        const labRequisitions = userRequisitions.filter(
          (r) =>
            r.type === "LAB_FACILITY" ||
            (r.labAccessDetails && r.labAccessDetails.some((l) => l.selected)),
        );

        const activeLabCount = labRequisitions.filter(
          (r) => r.status === "approved_provisioned",
        ).length;
        const pendingLabCount = labRequisitions.filter(
          (r) => r.status !== "approved_provisioned" && r.status !== "rejected",
        ).length;
            ...
            <div className="p-3.5 sm:p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 sm:gap-3 min-w-0">
              <div className="flex items-start sm:items-center gap-2.5 sm:gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">
                    Equipment Usage Authorization & Nodal Approvals across 9
              ...
              <div className="self-start sm:self-auto shrink-0 flex flex-wrap items-center gap-2">
                {activeLabCount > 0 ? (
                  <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 font-extrabold rounded-full text-xs border border-emerald-300 flex items-center gap-1">
                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />{" "}
                    {activeLabCount} Active Lab Access
                  </span>
                ) : pendingLabCount > 0 ? (
                  <span className="px-2.5 py-1 bg-amber-100 text-amber-800 font-extrabold rounded-full text-xs border border-amber-300 flex items-center gap-1">
                    <Clock className="w-3.5 h-3.5 text-amber-600 shrink-0" />{" "}
            ...
            <div className="p-3.5 sm:p-5 space-y-4 min-w-0">
              {labRequisitions.length === 0 ? (
                <div className="p-4 sm:p-6 text-center space-y-3 bg-slate-50 rounded-xl border border-dashed border-slate-300">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-800">
                      No Active Research Laboratory Access
                    </div>
                    <p className="text-[11px] text-slate-500 max-w-sm mx-auto mt-0.5 leading-relaxed">
                      Apply for equipment usage authorization across specialized
                      WII research labs (GIS & Remote Sensing, Wildlife
                      Forensics, Conservation Genetics, Analytical Suite, etc.).
                    </p>
                  </div>
                  <button
                    onClick={() => handleOpenModal("lab", "new")}
                ...
                labRequisitions.map((req, idx) => {
                  const selectedLabs =
                    req.labAccessDetails?.filter((l) => l.selected) || [];
                  ...
                  if (req.status === "approved_provisioned") {
                    return (
                      <div
                        key={`${req.id}-${idx}`}
                        className="p-3.5 sm:p-4 rounded-xl border border-emerald-200 bg-emerald-50/50 space-y-3 text-xs min-w-0"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                          <span className="font-bold text-emerald-900 flex items-center gap-1.5">
                            <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />{" "}
                            Active Authorized Laboratory Access
                          </span>
                          <div>
                            <button
                              onClick={() =>
                    ...
                    return (
                      <div
                        key={`${req.id}-${idx}`}
                        className="p-3.5 sm:p-4 rounded-xl border border-red-200 bg-red-50/50 space-y-2 text-xs min-w-0"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                          <span className="font-bold text-red-900 flex items-center gap-1.5 min-w-0">
                            <XCircle className="w-4 h-4 text-red-600 shrink-0" />{" "}
                            Lab Access Requisition Rejected
                          </span>
                        ...
                        <p className="text-slate-600 text-[11px] leading-relaxed">
                          ...
                          for <span className="font-semibold">{labNames}</span>{" "}
                          was rejected.
                        </p>
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-[11px] pt-1 min-w-0">
                          <span className="text-slate-500 font-medium">
                    ...
                    return (
                      <div
                        key={`${req.id}-${idx}`}
                        className="p-3.5 sm:p-4 rounded-xl border border-amber-200 bg-amber-50/50 space-y-2 text-xs min-w-0"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 min-w-0">
                          <span className="font-bold text-amber-900 flex items-center gap-1.5 min-w-0">
                            <Clock className="w-4 h-4 text-amber-600 shrink-0" />{" "}
                            Lab Access Requisition Pending Verification
                          </span>
                          <span className="self-start sm:self-auto px-2.5 py-0.5 bg-amber-100 text-amber-800 font-bold rounded-full text-[10px] border border-amber-300 shrink-0 whitespace-nowrap">
                            Under Workflow Review
                          </span>
                        ...
                        <p className="text-slate-600 text-[11px] leading-relaxed">
                          ...
                          for <span className="font-semibold">{labNames}</span>{" "}
                          has been submitted and is currently being verified by
                          your Supervising Officer / Nodal Officer.
                        </p>
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 text-[11px] pt-1 min-w-0">
                          <span className="text-slate-500 font-medium">
                            Requisition Ref:{" "}
