...
function normalizeStages(stages: unknown, type: WorkflowType, manager = ""): WorkflowStage[] {
...
function readLabRows(facility: any): LabFacilityRow[] {
  ...
  if (Array.isArray(configuredRows)) {
    return configuredRows.map((row: any, index: number) => ({
      id: String(row?.id || `LAB-${index + 1}`), name: String(row?.name || "").trim(), nodal: String(row?.nodal || "").trim(), assocNodal: String(row?.assocNodal || "").trim(), supervisor: String(row?.supervisor || "").trim(), status: String(row?.status || "active").toLowerCase() === "inactive" ? "inactive" : "active",
...
export function FacilitiesServicesSection({ facilitiesList, facilitiesLoading, facilitiesError, servicesList, servicesLoading, servicesError, fetchServices, fetchFacilities }: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const users = data.users.map((user: any) => ({ id: user.id, name: String(user.fullName || user.name || "").trim() })).filter((user: OfficerOption) => user.name);
        if (!cancelled) setOfficerOptions(users);
      } catch { /* Not Configured remains valid. */ }
    })();
  ...
  const displayServices = useMemo(() => {
    ...
    return DEFAULT_SERVICES.map((definition) => {
      const service = byId.get(definition.id) || {};
      return {
        ...definition,
        ...service,
        id: definition.id,
  ...
  const openAddRow = () => {
    setRowError(null);
    setEditingRow({ id: `LAB-${Date.now()}`, name: "", nodal: facility?.nodal || "Not Configured", assocNodal: facility?.assocNodal || "Not Configured", supervisor: facility?.supervisor || "Not Configured", status: "active" });
    setIsAddingRow(true);
  };
  const saveLabRow = async (event: React.FormEvent) => {
    ...
    const row = { ...editingRow, name: editingRow.name.trim() };
  ...
  const openWorkflowEditor = (type: WorkflowType, item: any) => {
    const stages = Array.isArray(item?.workflowStages) && item.workflowStages.length ? item.workflowStages : type === "facility" ? getDefaultFacilityWorkflow() : getDefaultServiceWorkflow(item?.manager || "");
    setWorkflowError(null);
    setEditingWorkflow({ type, item, stages: normalizeStages(stages, type, item?.manager || "") });
  };
  ...
  const addStage = () => {
    setEditingWorkflow((current) => {
      ...
      return { ...current, stages: [...current.stages, { stageNumber, stageName: "New Approval Stage", dealingRole: current.type === "facility" ? "supervisor" : "manager", dealingOfficerName: current.type === "facility" ? "Selected Lab Supervisor" : "Service In-Charge Manager", actionType: "verification", approvalMode: "all", isMandatory: true }] };
    });
  };
  ...
  const saveWorkflow = async () => {
    ...
    try {
      ...
      if (editingWorkflow.type === "facility") {
        const currentConfig = editingWorkflow.item.formConfig && typeof editingWorkflow.item.formConfig === "object" ? editingWorkflow.item.formConfig : {};
        await apiRequest(`/api/facilities/${encodeURIComponent(String(editingWorkflow.item.id))}`, { method: "PUT", body: JSON.stringify({ name: editingWorkflow.item.name || "Labs & Facility", dept: editingWorkflow.item.dept || null, nodal: editingWorkflow.item.nodal || "Not Configured", assocNodal: editingWorkflow.item.assocNodal || "Not Configured", supervisor: editingWorkflow.item.supervisor || "Not Configured", desc: editingWorkflow.item.desc || null, status: editingWorkflow.item.status || "active", workflowStages: stagesToSave, formConfig: { ...currentConfig, approvalWorkflowStages: stagesToSave } }) });
        await fetchFacilities();
      } else {
        const currentConfig = editingWorkflow.item.formConfig && typeof editingWorkflow.item.formConfig === "object" ? editingWorkflow.item.formConfig : {};
        await apiRequest(`/api/services/${encodeURIComponent(String(editingWorkflow.item.id))}`, { method: "PUT", body: JSON.stringify({ name: editingWorkflow.item.name, manager: editingWorkflow.item.manager || "Not Configured", quota: showQuotaAccess(editingWorkflow.item) ? editingWorkflow.item.quota || "" : "", status: editingWorkflow.item.status === "inactive" ? "inactive" : "active", workflowStages: stagesToSave, formConfig: { ...currentConfig, approvalWorkflowStages: stagesToSave } }) });
        await fetchServices();
      }
  ...
  const saveService = async () => {
    ...
    try {
      ...
      const workflowStages = Array.isArray(editingService.workflowStages) ? normalizeStages(editingService.workflowStages, "service", editingService.manager) : getDefaultServiceWorkflow(editingService.manager);
  ...
  const renderWorkflowPreview = (stages: WorkflowStage[], tone: "purple" | "emerald") => <div className="mt-3 border-t border-slate-200 pt-3"><div className="flex items-center gap-1 text-xs font-bold text-slate-700"><GitMerge className={`h-3.5 w-3.5 ${tone === "purple" ? "text-purple-600" : "text-emerald-600"}`} />Approval Flow ({stages.length} Stages)</div><div className="mt-2 flex flex-wrap items-center gap-1 text-[10px]">{stages.map((stage, index) => <React.Fragment key={`${stage.stageNumber}-${index}`}><span className={`rounded border px-1.5 py-0.5 font-medium ${tone === "purple" ? "border-purple-200 bg-purple-50 text-purple-800" : "border-emerald-200 bg-emerald-50 text-emerald-800"}`}>{String(stage.stageName).split(" ")[0]}</span>{index < stages.length - 1 && <span className="font-bold text-slate-400">➜</span>}</React.Fragment>)}</div></div>;

  return <div className="space-y-8">
    <section>
      <div className="mb-4 flex flex-wrap items-center justify-between border-b border-slate-200 pb-2"><div className="flex items-center gap-2"><Wrench className="h-4 w-4 text-emerald-600" /><h3 className="font-bold text-slate-800">Services Master Directory</h3><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] text-emerald-800">4 Total</span></div></div>
      ...
      {!servicesLoading && <div className="grid grid-cols-1 gap-5 md:grid-cols-3">{displayServices.map((service: any) => { const serviceStages = service.workflowStages?.length ? normalizeStages(service.workflowStages, "service", service.manager) : getDefaultServiceWorkflow(service.manager); const active = service.status !== "inactive"; return <div key={service.id} className={`rounded-xl border bg-white p-5 shadow-sm ${active ? "border-slate-200" : "border-slate-200 opacity-60"}`}><div className="flex items-start justify-between gap-3"><h4 className="text-base font-bold leading-snug text-slate-900">{service.name}</h4><span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${active ? "bg-emerald-100 text-emerald-800" : "bg-slate-100 text-slate-600"}`}>{active ? "Active" : "Inactive"}</span></div><div className="mt-4 space-y-2 text-sm"><div><span className="font-bold text-slate-700">Manager:</span> <span className="text-slate-600">{service.manager}</span><button type="button" onClick={() => setEditingService({ ...service })} className="ml-2 inline-flex rounded p-1 text-emerald-700 hover:bg-emerald-100" title="Edit Service"><Edit3 className="h-4 w-4" /></button></div>{showQuotaAccess(service) && <div><span className="font-bold text-slate-700">Quota / Access:</span> <span className="text-slate-600">{service.quota || "Not Configured"}</span></div>}</div>{renderWorkflowPreview(serviceStages, "emerald")}<div className="mt-2 flex justify-end"><button type="button" onClick={() => openWorkflowEditor("service", service)} className="rounded p-1.5 text-emerald-700 hover:bg-emerald-100" title="Configure Approval Flow"><GitMerge className="h-4 w-4" /></button></div></div>; })}</div>}
    </section>

    <section>
      <div className="mb-4 flex flex-wrap items-center justify-between border-b border-slate-200 pb-2"><div><h3 className="flex items-center gap-2 font-bold text-slate-800"><Building2 className="h-4 w-4 text-purple-600" />Labs & Facility Master <span className="rounded-full bg-purple-100 px-2 py-0.5 text-[10px] text-purple-800">{labRows.length} Total</span></h3><p className="mt-1 text-xs text-slate-500">Each lab/facility has its own NO, ANO and Supervisor. All rows follow the same common approval workflow.</p></div><button type="button" disabled={!facility} onClick={openAddRow} className="flex items-center gap-1.5 rounded-lg bg-purple-700 px-3 py-1.5 text-xs font-bold text-white hover:bg-purple-800 disabled:cursor-not-allowed disabled:opacity-50"><Plus className="h-3.5 w-3.5" />Add Lab / Facility</button></div>
