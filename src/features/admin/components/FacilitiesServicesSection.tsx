...
export function FacilitiesServicesSection({ facilitiesList, facilitiesLoading, facilitiesError, servicesList, servicesLoading, servicesError, fetchServices, fetchFacilities }: any) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Not Configured remains a valid selection.
      }
    })();
  ...
  const openWorkflowEditor = (type: WorkflowType, item: any) => {
    const stages = Array.isArray(item?.workflowStages) && item.workflowStages.length
      ? item.workflowStages
      : type === "facility" ? getDefaultFacilityWorkflow() : getDefaultServiceWorkflow(item?.manager || "");
  ...
  const addStage = () => {
    setEditingWorkflow((current) => {
      ...
      return {
        ...current,
        stages: [...current.stages, { stageNumber, stageName: "New Approval Stage", dealingRole: current.type === "facility" ? "supervisor" : "manager", dealingOfficerName: current.type === "facility" ? "Selected Lab Supervisor" : "Service In-Charge Manager", actionType: "verification", approvalMode: "all", isMandatory: true }],
      };
    });
  };
    ...
    <div className="space-y-8">
      <section>
        <div className="mb-4 flex flex-wrap items-center justify-between border-b border-slate-200 pb-2">
          <div className="flex items-center gap-2"><Wrench className="h-4 w-4 text-emerald-600" /><h3 className="font-bold text-slate-800">Services Master Directory</h3><span className="rounded-full bg-emerald-100 px-2 py-0.5 text-[10px] text-emerald-800">4 Total</span></div>
        </div>
        ...
        {!servicesLoading && <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          {displayServices.map((service: any) => {
            const serviceStages = service.workflowStages?.length ? normalizeStages(service.workflowStages, "service", service.manager) : getDefaultServiceWorkflow(service.manager);
            const active = service.status !== "inactive";
            return <div key={service.id} className={`rounded-xl border bg-white p-5 shadow-sm ${active ? "border-slate-200" : "border-slate-200 opacity-60"}`}>
              ...
              <div className="mt-4 space-y-2 text-sm"><div><span className="font-bold text-slate-700">Manager:</span> <span className="text-slate-600">{service.manager || "Not Configured"}</span><button type="button" onClick={() => setEditingService({ ...service })} className="ml-2 inline-flex rounded p-1 text-emerald-700 hover:bg-emerald-100" title="Edit Service"><Edit3 className="h-4 w-4" /></button></div>{showQuotaAccess(service) && <div><span className="font-bold text-slate-700">Quota / Access:</span> <span className="text-slate-600">{service.quota || "Not Configured"}</span></div>}</div>
              {renderWorkflowPreview(serviceStages, "emerald")}
              <div className="mt-2 flex justify-end"><button type="button" onClick={() => openWorkflowEditor("service", service)} className="rounded p-1.5 text-emerald-700 hover:bg-emerald-100" title="Configure Approval Flow"><GitMerge className="h-4 w-4" /></button></div>
            </div>;
          })}
        </div>}
      </section>
      ...
      <section>
        ...
        {facility && <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          {labRows.length === 0 && <div className="rounded-xl border border-dashed border-slate-300 bg-white p-10 text-center text-sm text-slate-500 md:col-span-2">No labs/facilities configured yet. Click <b>Add Lab / Facility</b> to add a row.</div>}
          {labRows.map((row) => <div key={row.id} className={`rounded-xl border bg-white p-5 shadow-sm ${row.status === "inactive" ? "border-slate-200 opacity-60" : "border-slate-200"}`}>
            <div className="flex items-start justify-between gap-3"><h4 className="text-base font-bold text-slate-900">{row.name}</h4><div className="flex items-center gap-1"><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${row.status === "inactive" ? "bg-slate-100 text-slate-600" : "bg-purple-100 text-purple-800"}`}>{row.status === "inactive" ? "Inactive" : "Active"}</span><button type="button" onClick={() => { setRowError(null); setEditingRow(row); setIsAddingRow(false); }} className="rounded p-1.5 text-purple-700 hover:bg-purple-100" title="Edit Lab / Facility"><Pencil className="h-4 w-4" /></button><button type="button" disabled={savingRows} onClick={() => void deleteLabRow(row)} className="rounded p-1.5 text-red-500 hover:bg-red-50 disabled:opacity-50" title="Remove Lab / Facility"><Trash2 className="h-4 w-4" /></button></div></div>
            <div className="mt-4 grid grid-cols-1 gap-2 text-sm sm:grid-cols-3"><div><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">NO</div><div className="text-slate-700">{row.nodal || "Not Configured"}</div></div><div><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">ANO</div><div className="text-slate-700">{row.assocNodal || "Not Configured"}</div></div><div><div className="text-[10px] font-bold uppercase tracking-wide text-slate-500">Supervisor</div><div className="text-slate-700">{row.supervisor || "Not Configured"}</div></div></div>
