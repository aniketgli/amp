  ...
  useEffect(() => {
    ...
    const loadOfficers = async () => {
      try {
        ...
        // Keep current values usable if the user directory is temporarily unavailable.
      }
    };
    loadOfficers();
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-8">
      ...
      <section>
        <div className="flex flex-wrap justify-between items-center border-b border-slate-200 pb-2 mb-4">
          ...
          <div className="flex items-center gap-2">{facility && <button onClick={() => handleOpenWorkflowModal("facility", facility)} className="px-3 py-1.5 text-purple-700 bg-purple-50 hover:bg-purple-100 rounded-lg text-xs font-bold flex items-center gap-1.5"><GitMerge className="w-3.5 h-3.5" />Approval Flow</button>}{facility && <button onClick={openAddRow} className="px-3 py-1.5 text-white bg-purple-700 hover:bg-purple-800 rounded-lg text-xs font-bold flex items-center gap-1.5"><Plus className="w-3.5 h-3.5" />Add Lab / Facility</button>}</div>
        </div>
