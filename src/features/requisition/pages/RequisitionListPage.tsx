...
export const RequisitionList: React.FC<RequisitionListProps> = ({
  ...
  const filtered = requisitions.filter((req) => {
    // Workflow Stage Forwarding Visibility Filter
    if (!isRequisitionVisibleForRole(req, currentRole)) {
      return false;
    }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-8">
        ...
        <div className="space-y-1.5 z-10 relative max-w-2xl min-w-0 flex-1">
          ...
          <h1 className="text-xl sm:text-2xl font-extrabold tracking-tight text-white leading-snug sm:leading-tight break-words">
            {initialTab === 'pending'
              ...
              : currentRole === 'lab_nodal'
              ? 'Research Lab Clearance Access Requests'
              ...
              : 'Master Access Management Portal'}
          </h1>
          ...
          <p className="text-xs text-slate-300 leading-relaxed max-w-xl block">
            {initialTab === 'pending'
              ...
              : 'Inspect, filter, and manage digital access requests for WII Email, Internet, HRMS, and Research Lab access.'}
          </p>
        </div>
      ...
      <div className="bg-white p-2.5 sm:p-3 rounded-2xl border border-slate-200/90 shadow-2xs flex items-center justify-between gap-2 text-xs min-w-0">
        <div className="relative min-w-0 flex-1 overflow-hidden">
          {/* Subtle Mobile Scroll Right Indicator Overlay */}
          <div className="pointer-events-none absolute right-0 top-0 bottom-0 w-6 bg-gradient-to-l from-white to-transparent sm:hidden z-10" />

          <div className="flex items-center gap-1 sm:gap-1.5 overflow-x-auto no-scrollbar py-0.5 min-w-0 flex-1 pr-8 sm:pr-1 touch-pan-x scroll-smooth">
            <button
              onClick={() => setActiveSubTab('all')}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 sm:px-3.5 sm:py-2 rounded-xl text-[11px] sm:text-xs font-bold transition-all cursor-pointer shrink-0 whitespace-nowrap ${
                activeSubTab === 'all'
      ...
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200/90 shadow-2xs flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 sm:gap-3 text-xs min-w-0">
        <div className="relative w-full sm:flex-1 min-w-0">
          <Search className="w-4 h-4 absolute left-3.5 top-2.5 text-slate-400 shrink-0" />
          <input
            type="text"
            placeholder="Search by Req ID, Applicant Name, PAN, Dept, PI, or Lab..."
            value={localSearch}
            onChange={(e) => setLocalSearch(e.target.value)}
            className="w-full pl-10 pr-3 py-2 text-xs border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-500 bg-slate-50/50"
          />
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 w-full sm:w-auto shrink-0 min-w-0">
          <div className="relative w-full sm:w-auto">
            <select
              ...
              className="w-full sm:w-auto px-3 py-2 border border-slate-300 rounded-xl text-slate-700 font-semibold bg-white text-xs truncate cursor-pointer"
            >
              <option value="all">All Service Types</option>
              <option value="COMBINED">Combined Access Request</option>
      ...
      {viewMode === 'table' ? (
        <div className="space-y-3">
          {/* Mobile Item Card List (< sm screens) */}
          <div className="block sm:hidden space-y-3">
            {filtered.length === 0 ? (
              <div className="bg-white p-8 rounded-2xl border border-slate-200 text-center space-y-2">
                <FileText className="w-8 h-8 text-slate-400 mx-auto" />
                <p className="text-sm font-bold text-slate-800">No Access Requests Match Criteria</p>
                <p className="text-xs text-slate-500">Try clearing search terms or selecting a different filter.</p>
              </div>
            ) : (
              filtered.flatMap((req, reqIdx) => {
                ...
                return accessItems.map((item, itemIdx) => {
                  ...
                  return (
                    <div
                      key={`mob-${req.id}-${item.key}-${reqIdx}-${itemIdx}`}
                      className={`bg-white rounded-2xl border p-3.5 shadow-2xs space-y-3 min-w-0 ${
                        needsAction ? 'border-amber-300 ring-2 ring-amber-300/20 bg-amber-50/10' : 'border-slate-200'
                      }`}
                    >
                      {/* Top bar: Ref ID, Mode Tag, Stage Badge */}
                      <div className="flex flex-wrap items-center justify-between gap-1.5 pb-2 border-b border-slate-100 min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="font-mono font-extrabold text-xs text-blue-700 truncate">{item.refId}</span>
                          <span
                            className={`px-1.5 py-0.5 text-[9px] font-black uppercase rounded border shrink-0 whitespace-nowrap ${
                        ...
                        <span className={`px-2 py-0.5 rounded-full text-[9px] font-semibold border shrink-0 whitespace-nowrap ${stageInfo.stageBadgeClass}`}>
                          {stageInfo.stageName}
                        </span>
                      </div>

                      {/* Access Item Label & Assigned Value */}
                      <div className="flex items-center justify-between gap-2 text-xs font-bold text-slate-800 min-w-0">
                        <div className="flex items-center gap-1.5 min-w-0">
                          {item.icon}
                          <span className="truncate">{item.label}</span>
                        </div>
                        {item.value && <div className="shrink-0">{item.value}</div>}
                      </div>

                      {/* Applicant & PI Details if non-user role */}
                      {!isUserRole && (
                        <div className="bg-slate-50 p-2.5 rounded-xl text-[11px] space-y-1 border border-slate-200/80 min-w-0">
                          <div className="flex justify-between items-center text-slate-700 min-w-0 gap-1">
                            <span className="text-slate-400 text-[10px] uppercase font-bold shrink-0">Applicant:</span>
                            <span className="font-bold text-slate-900 truncate">{req.applicant.applicantName}</span>
                          </div>
                          <div className="flex justify-between items-center text-slate-600 min-w-0 gap-1">
                            <span className="text-slate-400 text-[10px] uppercase font-bold shrink-0">Dept/Cell:</span>
                            <span className="font-medium truncate">{req.applicant.departmentCellProject}</span>
                          </div>
                          {(isItHeadRole || isManagerRole) && (
                            <div className="flex justify-between items-center text-slate-600 min-w-0 gap-1 pt-1 border-t border-slate-200/60">
                              <span className="text-slate-400 text-[10px] uppercase font-bold shrink-0">PI:</span>
                              <span className="font-semibold text-slate-800 truncate">{req.applicant.supervisingOfficerName}</span>
                            </div>
                          )}
                        </div>
                      )}

                      {/* Dates & Dealing Officer */}
                      <div className="grid grid-cols-2 gap-2 text-[10px] bg-slate-50/70 p-2 rounded-xl border border-slate-100">
                        <div>
                          <span className="text-slate-400 block font-bold uppercase text-[9px]">From Date</span>
                          <span className="font-mono font-bold text-slate-800">{item.fromDate}</span>
                        </div>
                        <div>
                          <span className="text-slate-400 block font-bold uppercase text-[9px]">To Date</span>
                          <span className="font-mono font-bold text-slate-800">{item.toDate}</span>
                        </div>
                        <div className="col-span-2 pt-1 border-t border-slate-200/60 flex items-center justify-between text-slate-600">
                          <span className="text-slate-400 font-bold uppercase text-[9px] shrink-0">Dealing Officer:</span>
                          <span className="font-medium text-slate-800 truncate pl-1">{stageInfo.dealingOfficer} ({stageInfo.officerRole})</span>
                        </div>
                      </div>
                      ...
                      <div className="pt-2 border-t border-slate-100 flex items-center justify-end gap-2">
                        {isPiRole && needsAction && (
                          <button
                            ...
                            className="px-2.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold transition-all cursor-pointer shadow-2xs flex items-center gap-1"
                          >
                        ...
                        {isManagerRole && needsAction && (
                          <button
                            onClick={() => setSelectedForAction({ req, decision: 'approve' })}
                            className="px-2.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-xs font-bold transition-all cursor-pointer shadow-2xs flex items-center gap-1"
                          >
                            <SlidersHorizontal className="w-3.5 h-3.5" /> Action
                          </button>
                        )}
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 rounded-lg text-xs font-bold shadow-2xs transition-all cursor-pointer flex items-center gap-1"
                        >
          ...
          {/* Desktop/Tablet Table Mode View (Hidden on mobile) */}
              ...
              <table className="w-full text-left border-collapse text-xs min-w-[700px]">
                <thead>
                  <tr className="bg-[#f8fafc] text-slate-600 border-b border-slate-200 text-[10px] uppercase font-extrabold tracking-wider">
                    <th className="py-2.5 px-2.5 text-slate-600 whitespace-nowrap min-w-[110px]">Req ID & Date</th>
                    {!isUserRole && <th className="py-2.5 px-2.5 text-slate-600 whitespace-nowrap min-w-[120px]">Applicant</th>}
                    {(isItHeadRole || isManagerRole) && <th className="py-2.5 px-2.5 text-slate-600 whitespace-nowrap min-w-[120px]">Supervising PI</th>}
                    <th className="py-2.5 px-2.5 text-slate-600 whitespace-nowrap min-w-[130px]">Access Requested</th>
                    <th className="py-2.5 px-2.5 text-slate-600 whitespace-nowrap min-w-[80px]">From Date</th>
                    <th className="py-2.5 px-2.5 text-slate-600 whitespace-nowrap min-w-[80px]">To Date</th>
                    <th className="py-2.5 px-2.5 text-slate-600 whitespace-nowrap min-w-[130px]">Stage & Officer</th>
                    <th className="py-2.5 px-2.5 text-center text-slate-600 whitespace-nowrap min-w-[100px] sticky right-0 bg-[#f8fafc] z-10 border-l border-slate-200/80 shadow-[-4px_0_8px_-2px_rgba(0,0,0,0.05)]">Actions</th>
                  </tr>
                </thead>
                  ...
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={isUserRole ? 6 : isPiRole ? 7 : 8} className="p-10 text-center space-y-3">
                        ...
                        <div className="text-sm font-bold text-slate-800">No Access Requests Match Criteria</div>
                      ...
                      return accessItems.map((item, itemIdx) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* 4. Access Requested */}
                            <td className="py-2 px-2.5 align-middle">
                              <div className="flex items-center gap-1.5 text-[11px] font-semibold text-slate-800 whitespace-nowrap">
                                {item.icon}
                            ...
                            <td className="py-2 px-2.5 align-middle">
                              <div className="flex flex-col gap-1 text-[10px]">
                                ...
                                <div className="flex items-center gap-1 text-slate-500 whitespace-nowrap" title={`Dealing Officer: ${stageInfo.dealingOfficer} (${stageInfo.officerRole})`}>
                                  ...
                                  <span className="font-medium text-slate-700 truncate max-w-[120px]">{stageInfo.dealingOfficer}</span>
                            ...
                            <td className={`py-2 px-2.5 align-middle text-center sticky right-0 z-10 border-l border-slate-200/80 shadow-[-4px_0_8px_-2px_rgba(0,0,0,0.05)] ${
                              needsAction ? 'bg-[#fffbeb]' : 'bg-white'
                            }`}>
                              <div className="flex items-center justify-center gap-1 shrink-0">
                                {isPiRole && needsAction && (
                                  <button
                                    onClick={() => setSelectedForAction({ req, decision: 'approve' })}
                                    className="px-2 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-md text-[10px] font-bold transition-all cursor-pointer shadow-2xs flex items-center gap-0.5 whitespace-nowrap"
                                ...
                                {isItHeadRole && needsAction && (
                                  ...
                                  >
                                    <ShieldCheck className="w-3 h-3 shrink-0" /> Authorize
                                  </button>
                                )}
                                ...
                                {isManagerRole && needsAction && (
                                  <button
                                    onClick={() => setSelectedForAction({ req, decision: 'approve' })}
                                    className="px-2 py-1 bg-indigo-600 hover:bg-indigo-500 text-white rounded-md text-[10px] font-bold transition-all cursor-pointer shadow-2xs flex items-center gap-0.5 whitespace-nowrap"
                                    title="Action & Provision Access"
                                  >
                                    <SlidersHorizontal className="w-3 h-3 shrink-0" /> Action
                                  </button>
                                )}
                                ...
                                <button
                                  ...
                                  className="px-2 py-1 bg-white hover:bg-slate-100 text-slate-700 border border-slate-300 rounded-md text-[10px] font-bold shadow-2xs transition-all cursor-pointer flex items-center gap-0.5 whitespace-nowrap"
                                  title="View Full Requisition Details"
                                >
                                  <Eye className="w-3 h-3 text-slate-500 shrink-0" /> View
                                </button>
                              </div>
                            </td>
                          </tr>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.length === 0 ? (
            <div className="col-span-full bg-white p-10 rounded-2xl border border-slate-200 text-center space-y-2">
              <FileText className="w-8 h-8 text-slate-400 mx-auto" />
              <p className="text-sm font-bold text-slate-800">No Access Requests Found</p>
            </div>
          ) : (
