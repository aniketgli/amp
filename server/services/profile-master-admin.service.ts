...
const ORG_UNIT_TYPES = new Set([
  "department",
  "cell",
  "project",
]);

const BATCH_SERIES_TYPES = new Set([
  "msc",
  "diploma_trainee",
]);
...
function validateYear(
  ...
  if (
    ...
    throw new Error(
      `${fieldName} must be between 1900 and 2200.`,
    );
  }
...
async function getSeriesType(
  ...
  return rows?.[0]?.series_type
    ? String(rows[0].series_type)
    : null;
}
...
export async function addOrgUnit(
  ...
  const unitType =
    cleanString(input?.unitType)
      .toLowerCase();

  if (
    !ORG_UNIT_TYPES.has(unitType)
  ) {
    throw new Error(
  ...
  if (
    ...
    throw new Error(
      "An organization unit with the same name already exists.",
    );
  }
...
export async function addBank(
  ...
  if (
    ...
    throw new Error(
      "Bank Code cannot exceed 50 characters.",
    );
  }

  if (
    await bankExists(bankName)
  ) {
    throw new Error(
...
export async function changeBankStatus(
  idInput: unknown,
  statusInput: unknown,
) {
  const id =
...
export async function addBatch(
  ...
  if (
    !(await seriesExists(seriesId))
  ) {
    throw new Error(
      "Selected batch series is invalid or inactive.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      "This batch number already exists in the selected series.",
    );
  }
