...
export function registerFacilitiesRoutes(app: Express) {
  ...
  app.post("/api/facilities", authenticateToken, requireRole(...ADMIN_ROLES), async (req, res) => {
    try {
      const { name, dept, nodal, assocNodal, supervisor, desc, status = "active", workflowStages = null, formConfig = null } = req.body;
      if (!name || !nodal || !assocNodal || !supervisor) return res.status(400).json({ success: false, message: "Facility name, Nodal Officer, Associate Nodal Officer and Supervisor are required." });
      if (!isDbConnected) return res.status(503).json({ success: false, message: "Database is unavailable." });
      const facilityId = await getNextFacilityId();
      await createFacility({ id: facilityId, name: String(name).trim(), dept: dept || null, nodal: String(nodal).trim(), assocNodal: String(assocNodal).trim(), supervisor: String(supervisor).trim(), desc: desc || null, status, workflowStages, formConfig });
  ...
  app.put("/api/facilities/:id", authenticateToken, requireRole(...ADMIN_ROLES), async (req, res) => {
    try {
      ...
      if (!name || !nodal || !assocNodal || !supervisor) return res.status(400).json({ success: false, message: "Facility name, Nodal Officer, Associate Nodal Officer and Supervisor are required." });
      if (!isDbConnected) return res.status(503).json({ success: false, message: "Database is unavailable." });
      const updated = await updateFacility({ id, name: String(name).trim(), dept: dept || null, nodal: String(nodal).trim(), assocNodal: String(assocNodal).trim(), supervisor: String(supervisor).trim(), desc: desc === undefined ? null : desc, status, workflowStages, formConfig });
